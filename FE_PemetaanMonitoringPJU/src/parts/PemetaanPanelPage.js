...
const PemetaanPanelPage = () => {
  ...
  const fetchRiwayatPanel = async (panelId) => {
    ...
    try {
      const response = await axios.get(`http://localhost:8000/api/riwayat-panel-users/${panelId}`);

      // Handle response format
      const { riwayat_panel, pengaduan_detail } = response.data;

      const combinedData = [];
      
      // Tambahkan riwayat panel jika ada
      if (riwayat_panel) {
        combinedData.push({
          ...riwayat_panel,
          id_riwayat_panel: `riwayat-${riwayat_panel.id_riwayat_panel}`, // Pastikan unique key
          source: "Riwayat",
        });
      }

      // Tambahkan pengaduan jika ada
      if (pengaduan_detail) {
        combinedData.push({
          id_riwayat_panel: `pengaduan-${pengaduan_detail.id_detail_pengaduan}`,
          lokasi: pengaduan_detail.pengaduan?.lokasi || "",
          tanggal_masalah: pengaduan_detail.pengaduan?.tanggal_pengaduan || "",
          jam_masalah: pengaduan_detail.pengaduan?.jam_aduan || "",
          keterangan_masalah: pengaduan_detail.pengaduan?.keterangan_masalah || "",
          uraian_masalah: pengaduan_detail.pengaduan?.uraian_masalah || "",
          tanggal_penyelesaian: pengaduan_detail.pengaduan?.tanggal_penyelesaian || "",
          jam_penyelesaian: pengaduan_detail.pengaduan?.jam_penyelesaian || "",
          durasi_penyelesaian: pengaduan_detail.pengaduan?.durasi_penyelesaian || "",
          penyelesaian_masalah: pengaduan_detail.pengaduan?.penyelesaian_masalah || "",
          pencegahan: pengaduan_detail.pengaduan?.pencegahan_masalah || "",
          nomor_rujukan: pengaduan_detail.pengaduan?.nomor_pengaduan || "",
          status: pengaduan_detail.pengaduan?.status || "",
          source: "Pengaduan",
        });
      }
